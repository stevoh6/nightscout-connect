...
function glookoSource(opts, axios) {
  ...
  const impl = {
    ...
    async dataFromSesssion(session, last_known) {
      ...
      const endDate = opts.glookoEndDate ? new Date(opts.glookoEndDate) : new Date();
      const startDate = opts.glookoStartDate ? new Date(opts.glookoStartDate) : new Date(twoDaysAgoMs);
