...
/**
 * Convert Glooko batch into Nightscout treatments.
 *
 * @param {any} batch
...
function generate_nightscout_treatments(batch, timestampDeltaMs, logger) {
  const foods = batch?.foods || batch?.food;
  ...
  if (foodsArr.length) {
    foodsArr.forEach((element) => {
      ...
      if (insulin) {
        ...
        treatment.created_at = treatment.eventTime;
  ...
  // Scheduled basals -> Temp Basal (existing behavior)
