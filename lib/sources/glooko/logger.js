...
function redactHeaders(headers = {}) {
  ...
  for (const k of Object.keys(out)) {
    const key = k.toLowerCase();
    if (key.includes('authorization') || key.includes('cookie') || key.includes('token') || key.includes('api-key') || key.includes('api-secret')) {
      out[k] = '[REDACTED]';
    }
  }
  return out;
...
function createLogger(opts = {}) {
  ...
  function emit(lvlName, msg, fields) {
    ...
    // JSON lines make logs easy to grep + share
